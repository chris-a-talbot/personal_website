...
const ParticleSystem = ({ progress }: ParticleSystemProps) => {
    ...
    useEffect(() => {
        ...
        const createPetalOutlinePoint = (petalIndex: number, progress: number) => {
            const petalCount = 5;
        ...
        const animate = () => {
            ...
            for (let i = 0; i < PARTICLE_COUNT; i++) {
                ...
                const color = new THREE.Color();
                color.setHSL(
                    baseHues[i] + shimmerOffset,
                    0.7 + Math.sin(time * 3 + i * 0.05) * 0.1,
                    0.5 + Math.sin(time * 4 + i * 0.15) * 0.1
                );
